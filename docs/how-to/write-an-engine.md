...
1. Read the [engine contract](../explanation/engine-contract.md) for ownership
   and lifecycle.
2. Read the [`typio-engine-protocol` contract](../../crates/typio-engine-protocol/README.md).
3. Copy the minimal
   [`hello_worker`](../../crates/typio-engine-protocol/examples/hello_worker.rs) shape
...
7. Run `typio-engine-check <manifest>` in the engine's test and packaging workflow.
