...
The Typio daemon speaks **TIP v3** — JSON-RPC 2.0 with length-prefixed frames
...
# Switch the active keyboard engine. Activation is modality-explicit:
# the daemon accepts keyboard.use and voice.use, not the retired engine.use.
# The CLI resolves the kind from engine.list and dispatches to the matching verb.
tip_call("keyboard.use", {"name": "rime"})
...
- [IPC Protocol Reference](../reference/ipc-protocol.md) — Complete method catalog.
- [Command-Line Interface Reference](../reference/cli.md) — `typioctl` command to RPC mapping.
