...
| Version | `1.0` |
| Magic | `TYEP` |
| Rust contract | [`typio-engine-protocol`](../../crates/typio-engine-protocol/README.md) |
| Runtime backend | `typio_runtime::core::engine::backend::ProcessBackend` |
...
| `RESULT` | `NOT_HANDLED`, `HANDLED`, or `PASS_THROUGH`; the legacy aliases `COMPOSING` and `COMMITTED` decode to `HANDLED` |
