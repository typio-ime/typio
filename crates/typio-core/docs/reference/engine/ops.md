...
1. **`TypioEngineBaseOps`** — the base vtable for **every** engine.
2. **`TypioKeyboardEngineOps`** — mandatory only for **keyboard** engines.
3. **`TypioVoiceEngineOps`** — mandatory only for **voice** engines.
4. **`TypioEngineSurfaceOps`** — optional, for engines that expose commands.
...
|----------|-------------|-------------------|
| `init` | Once during worker startup, after EngineHello and HostHello. Receives the worker-local `TypioInstance`. | Allocate state via `typio_engine_set_user_data` and read unified config through `typio_instance_get_config`. Return `TYPIO_OK` on success. |
| `destroy` | Once, as part of `typio_engine_free`. The worker harness owns this call. | Free every resource allocated in `init` (including `user_data`). The helper frees the engine struct. |
...
| `reload_config` | The host forwarded a config reload. | Re-read engine-owned settings from the worker-local `TypioConfig`. Return `TYPIO_OK` even if no engine-specific config exists. |
...
The base vtable itself is required, but each callback may be `NULL`. The
worker treats missing lifecycle callbacks as no-ops, missing reload as success,
and missing availability as `TYPIO_ENGINE_READY`.
...
`process_audio` runs while the worker services a bounded protocol request. It
may block for inference, but must not start concurrent writes to the protocol
fd or retain the borrowed sample buffer.

## Surface operations (optional)

Engines that expose invokable actions populate `TypioEngineSurfaceOps` and
attach it via `typio_engine_set_surface_ops`
([entry.md](entry.md#utility-accessors)). Runtime-settable values belong in the
engine config schema.

```c
typedef struct TypioEngineSurfaceOps {
    const TypioEngineCommand *(*list_commands)(TypioEngine *engine,
                                                size_t *out_count);
...
See [Types ▸ Surface types](types.md#surface-ops-types) for
`TypioEngineCommand`.

The worker serialises this vtable through `list-commands` and
`invoke-command` protocol operations, so the host, CLI, and settings client
can invoke engine actions without per-engine code.
