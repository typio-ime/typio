...
`typio-core` is Typio's platform-neutral Rust runtime. It owns configuration,
input contexts, engine registry policy, language and mode selection, and the
out-of-process engine backend.

The crate builds only an `rlib` and is embedded by `typio-host`. It publishes no
...
```bash
cargo build -p typio-core
cargo test -p typio-core
```

See [the core documentation](docs/index.md) and
