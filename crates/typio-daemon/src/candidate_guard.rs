...
//!
//! When an engine publishes a composition with host-managed-selection flags
//! set, the host intercepts the corresponding navigation/selection keys before
//! they reach the engine's `process_key`. The host manages the selected index
//! locally and commits via `commit_candidate`.
//!
//! ## Scope
//!
...
//!
//! The effectful commit path is not part of this pure module. The resolution is
//! in [`host_selection_resolve`]; the owned context operation lives in
//! [`KeyboardRouter::try_host_selection`] in `router.rs` and falls back to
//! normal engine key processing when candidate commit is declined.
//!
//! ## Opt-in semantics
//!
//! The host never intercepts navigation keys on its own. It requires explicit
//! opt-in via a non-empty `TypioHostManagedSelection` flag set, so engines that
//! do not declare the capability keep receiving every key. See
//! [`should_consume_key`].
...
/// Host-managed-selection key code.
...
/// candidates).
///
/// `None` means "no resolution" (index pick beyond the candidate count,
/// a non-resolution key, or zero candidates); otherwise the target index.
...
///    into.
///
/// **Opt-in only.** An empty `flags` never intercepts, so the host does not
/// take candidate navigation away from an engine that manages selection
/// itself. An engine that wants host-managed selection must explicitly
/// declare a non-zero `TypioHostManagedSelection` flag (ADR-0012).
