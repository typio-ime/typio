...
//!
//! The controller sits between the runtime layer and external surfaces (tray,
//! D-Bus status bus): it maintains a snapshot of user-visible state, answers
//! queries from ONE place instead of every surface reaching into
//! `TypioInstance`, and broadcasts change notifications so surfaces update
//! uniformly.
//!
//! Runtime registry/config reads are abstracted behind [`RegistryView`] so the
//! notify/broadcast logic is unit-testable without a live instance; the
//! production adapter wraps `TypioInstance`.
