//! System-tray helpers.
//!
//! Free functions that translate between the live typio-runtime registry and
...
pub(super) fn cycle_active_keyboard(instance: &SharedInstance) {
    if let Some(mut registry) = instance.borrow_mut().registry_rust_mut() {
        let _ = registry.switch_keyboard(typio_runtime::core::registry::SwitchDirection::Next);
    }
}
...
/// for the target language (typio-runtime's per-language memory). When fewer than
...
pub(super) fn cycle_active_language(instance: &SharedInstance) {
    let result = {
        instance
            .borrow_mut()
            .cycle_language(typio_runtime::core::registry::SwitchDirection::Next)
    };
    match result {
        Ok(()) => {
