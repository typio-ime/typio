...
#[cfg(feature = "wayland")]
use typio_runtime::voice::types::VoiceState;
...
pub(super) struct RegistryLabelSources<'a> {
    pub(super) registry: &'a typio_runtime::core::registry::EngineRegistry,
}
...
impl App {
    /// Read `display.indicator_*` from typio-runtime's config cache. Returns
    ...
    /// live registry and cached mode from typio-runtime; applies the salience
    ...
    fn trigger_indicator_show(&mut self, path: IndicatorPath) {
        ...
        // Read the current mode from typio-runtime's cache. The mode-changed
        ...
        let label = {
            ...
            // Build the mode snapshot from the typio-runtime-cached mode. The
    ...
    /// Read the cached mode's `display_label` from typio-runtime (e.g. "中",
    ...
    fn read_mode_salience(&self) -> Salience {
        ...
        match instance.last_keyboard_mode().map(|mode| mode.salience) {
            Some(typio_runtime::core::engine::ModeSalience::Notable) => Salience::Notable,
            _ => Salience::Quiet,
        }
    }
