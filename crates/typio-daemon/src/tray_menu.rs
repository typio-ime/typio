//! Pure in-memory model of the tray dbusmenu, decoupled from D-Bus transport.
//!
//! The model captures WHAT the tray menu contains at the current instant
//! (items, labels, radio state, submenu nesting). [`build`] turns a registry
//! snapshot into the tree; serialising the tree to a dbusmenu `GetLayout`
//! reply is a separate concern (see [`crate::tray_sni`]). Splitting the two
//! makes the menu structure unit-testable without a D-Bus fixture.
//!
//! The builder's input is a plain-data [`RegistrySnapshot`], so the ADR-0033
//! layout logic is pure and testable; a thin adapter populates the snapshot
