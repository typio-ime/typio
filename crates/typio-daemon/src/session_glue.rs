...
pub(crate) trait ApplyTarget {
    ...
    fn commit(&mut self);
    /// 6. Reset keyboard routing ownership.
    fn reset_key_routing(&mut self);
    /// 7. Create grab.
    ...
    )
{
    ...
    fn reactivate(&mut self) {
        ...
        // field. Reset its gesture ownership; the ordered keyboard boundary
        // pairs virtual-keyboard releases in the input driver. Keep
        // pending composition text intact along with the engine composition.
        self.1.fence_key_routing();
        let _ = self.2.stop();
        let state = self.0.state_mut();
        state.text_input_rect = None;
        state.clear_caret_rect();
...
fn apply(effects: &EffectSet, target: &mut dyn ApplyTarget) {
    ...
    if effects.reset_key_routing {
        target.reset_key_routing();
    }
