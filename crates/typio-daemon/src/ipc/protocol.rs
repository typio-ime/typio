...
//!
//! Method/topic names are `&'static str` constants matching the wire
//! vocabulary exactly; the `typioctl` client compares against these literals, so
//! renaming a constant here is a wire-incompatible change.
...
///
/// The path is returned by value as an owned [`PathBuf`].
