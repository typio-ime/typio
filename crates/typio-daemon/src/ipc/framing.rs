...
//!
//! Builds the request/response/notification/error envelope with `serde_json`
//! and `serde` derives, so the wire shape is defined once by Rust types.
//!
//! ## What this module does not define
//!
//! Per-method typed `params` / `result` structs (e.g. `HelloParams`,
//! `ConfigGetResult`). Handlers dispatch per-method using ad-hoc JSON values;
//! per-method typed structs land alongside each handler subsystem.
//! corresponding handler port (config access, engine registry, etc.).
//! Until then, `params` and `result` are untyped
//! [`serde_json::Value`]s for the per-method payloads.
//!
