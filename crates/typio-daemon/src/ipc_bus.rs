...
//!
//! The bus is the thin gluing layer between transport ([`UdsServer`]) and
//! dispatch ([`StatusService`]): it installs the request handler, forwards
//! subscription changes to the notifier, and supplies a runtime-backed
...
// ── typio-runtime-backed ServiceBackend ─────────────────────────────────────────
...
impl TypioBackend {
    ...
    fn with_registry<R>(
        &self,
        operation: impl FnOnce(&typio_runtime::core::registry::EngineRegistry) -> R,
    ) -> Option<R> {
    ...
    fn with_registry_mut<R>(
        &self,
        operation: impl FnOnce(&mut typio_runtime::core::registry::EngineRegistry) -> R,
    ) -> Option<R> {
        let instance = self.instance.borrow();
        instance
            .registry_rust_mut()
...
impl ServiceBackend for TypioBackend {
    ...
    fn config_unset(&mut self, key: &str) -> Option<Result<(), SvcError>> {
        ...
        if cfg.remove(key) {
            typio_runtime::config_schema::apply_defaults(cfg);
            Some(Ok(()))
        } else if has_schema {
            Some(Ok(()))
        } else {
    ...
    fn config_list(&self, prefix: &str) -> Option<Vec<ConfigEntry>> {
        ...
        for schema in typio_runtime::config_schema::fields() {
    ...
    fn engine_info(&self, name: &str) -> Option<EngineKind> {
        self.with_registry(|r| {
            r.engine_info(name).map(|info| match info.engine_type {
                typio_runtime::core::engine::EngineType::Keyboard => EngineKind::Keyboard,
                typio_runtime::core::engine::EngineType::Voice => EngineKind::Voice,
            })
        })
        .flatten()
    }
    ...
    fn cycle_language(&mut self, forward: bool) -> CycleLanguageOutcome {
        let direction = if forward {
            typio_runtime::core::registry::SwitchDirection::Next
        } else {
            typio_runtime::core::registry::SwitchDirection::Previous
        };
        ...
        match result {
            ...
            Err(typio_runtime::core::engine::EngineError::NotFound) => {
                CycleLanguageOutcome::NoLanguages
            }
    ...
    fn invoke_command(&mut self, name: &str, cmd: &str) -> InvokeOutcome {
        match self.with_registry_mut(|registry| registry.invoke_command(name, cmd)) {
            ...
            Some(Err(typio_runtime::core::engine::EngineError::NotFound)) => {
                InvokeOutcome::NotFound
            }
            Some(Err(typio_runtime::core::engine::EngineError::NotSupported)) => {
    ...
    fn engine_reload(&mut self, name: &str, path: Option<&str>) -> Result<(), SvcError> {
        ...
        let mut scratch = typio_runtime::core::registry::EngineRegistry::new();
...
impl SchemaField {
    fn from_owned(field: &typio_runtime::config_schema::ConfigSchemaField) -> Self {
        use typio_runtime::config_schema::ConfigDefault;
        let (field_type, default) = match &field.default {
        ...
        Self {
            ...
            choices: (!field.options.is_empty()).then(|| field.options.clone()),
            min: field.minimum,
            max: field.maximum,
        }
    }
...
fn schema_field(key: &str) -> Option<SchemaField> {
    typio_runtime::config_schema::find(key).map(|field| SchemaField::from_owned(&field))
}
...
fn config_value_type(value: &typio_runtime::config::ConfigValue) -> Option<FieldType> {
    use typio_runtime::config::ConfigValue;
    match value {
...
fn config_value_to_json(value: &typio_runtime::config::ConfigValue) -> Value {
    use typio_runtime::config::ConfigValue;
    match value {
...
fn set_config_value(
    config: &mut typio_runtime::config::Config,
    key: &str,
    raw: &str,
    field_type: FieldType,
    ...
    match field_type {
        ...
        FieldType::Int => {
            ...
            config.set(key, typio_runtime::config::ConfigValue::Int(value));
        }
        FieldType::Bool => {
            ...
            config.set(key, typio_runtime::config::ConfigValue::Bool(value));
        }
        FieldType::Float => {
            ...
            config.set(key, typio_runtime::config::ConfigValue::Float(value));
        }
        FieldType::Array => {
            let values = parse_string_list(raw)?;
            config.set(
                key,
                typio_runtime::config::ConfigValue::Array(
                    values
                        .into_iter()
                        .map(typio_runtime::config::ConfigValue::String)
                        .collect(),
                ),
            );
        }
...
mod tests {
    ...
    fn live_backend_reports_schema_types_and_sources() {
        ...
        let mut instance =
            typio_runtime::TypioInstance::new_rust(Some(path), Some(path), Some(path), Vec::new());
...
"#
        ...
        let path = temp.path().to_str().unwrap();
        let mut instance = typio_runtime::TypioInstance::new_rust(
            Some(path),
            Some(path),
            Some(path),
            vec![path.to_string()],
