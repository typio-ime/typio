...
//!
//! The registry exposes only the raw BCP-47 tag; the presentation table lives
//! here because a tag-to-display mapping is a frontend concern. Everything in
//! this module is pure over its inputs — no `TypioInstance`, no registry, no
//! config object — so it is unit-testable without fixtures. The stateful
//! controller (listeners, snapshot, instance wiring) is
...
/// independent of the typio-runtime config object.
