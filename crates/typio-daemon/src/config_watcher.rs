...
//!
//! The watcher is mechanism only: it does not perform frontend side effects
//! (purge font caches, invalidate the candidate panel, reload shortcut
//! config, switch voice engine). The caller receives a reload trigger via
//! [`ConfigWatcher::drain_timer`] returning `true` and decides what to do
//! about it.
//!
//! ## Architecture
//!
...
impl ConfigWatcher {
    ...
    /// directory and filters events to the relevant files.
