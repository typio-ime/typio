...
//! trait. The real typio-runtime-backed implementation is a separate concern; here the
...
/// subscribe callback; the UDS transport supplies the real value.
...
pub trait ServiceBackend {
    ...
    /// The full TOML text. Empty string when unavailable: `config.show`
    /// reports absence as an empty document rather than an error.
...
impl<B: ServiceBackend> StatusService<B> {
    ...
    /// Reset the uptime origin (normally set at construction; tests use this
    /// to make uptime deterministic).
