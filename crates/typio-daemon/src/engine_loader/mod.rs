...
//! registers the engines they describe with typio-runtime's [`EngineRegistry`].
//!
//! Manifest discovery is typed and sits on top of typio-runtime's native
//! registry API. Startup and `engine.reload` both use this loader, so
//! validation and schema probing follow one path.
//!
//! ## Architecture
//!
...
use typio_runtime::core::engine::backend::ProcessBackend;
use typio_runtime::core::engine::{EngineCapabilities, EngineInfo, EngineType};
use typio_runtime::core::registry::EngineRegistry;
...
///
/// Instance-scoped rather than global, so multiple loaders can coexist
/// (which is what makes the loader testable).
#[derive(Debug, Clone)]
...
impl EngineLoader {
    ...
    ///
    /// Errors opening the directory are downgraded to "0 engines
    /// registered" (logged at debug level); per-manifest errors are returned
    /// individually via [`LoadOutcome`] in the returned log but do not abort
    /// the scan.
    ...
    ///   7. Build a [`ProcessBackend`] and register it via typio-runtime's native
    ...
    pub fn load_single(
        ...
        // Register via typio-runtime's native Rust API.
        ...
        if let Err(err) = registry.register(backend) {
            match err {
                typio_runtime::core::engine::EngineError::AlreadyExists => {
                    return Err(LoadError::Skipped(SkipReason::AlreadyRegistered(
                        manifest.name,
                    )));
                }
...
///
/// Skips are non-fatal: they are surfaced in [`LoadDirReport::skipped`] for
/// the caller to decide how to report, and the scan continues.
#[derive(Debug, Clone)]
pub enum SkipReason {
...
/// the system, or typio-runtime — not a deliberate skip.
#[derive(Debug)]
pub enum LoadError {
    ...
    RegisterFailed(typio_runtime::core::engine::EngineError),
    /// `EngineRegistry::set_engine_languages` failed after registration.
    SetLanguagesFailed(typio_runtime::core::engine::EngineError),
    /// The executable produced an invalid or incompatible HELLO/schema.
    SchemaProbeFailed(typio_runtime::core::engine::EngineError),
