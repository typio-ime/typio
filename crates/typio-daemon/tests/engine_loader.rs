...
//! each parses, validates, and registers successfully into typio-runtime's
...
//! field, different value type, etc.), this test fails before the daemon
//! loader is retired.

use std::fs;
use std::path::PathBuf;

use typio_daemon::engine_loader::{EngineLoader, LoadError, SkipReason};
...
fn engine_workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR = .../typio/crates/typio-daemon
...
fn all_real_manifests_parse_and_validate() {
    ...
    for (engine_dir, manifest_name) in REAL_MANIFESTS {
        let Some(path) = resolve_manifest_path(engine_dir, manifest_name) else {
            eprintln!("skipping {engine_dir}: manifest {manifest_name} not present",);
            continue;
        };
        let m = typio_daemon::engine_loader::manifest::EngineManifest::read_from(&path)
...
#[test]
fn all_real_manifests_register_into_typio_runtime() {
    ...
    loader.set_capabilities(typio_daemon::engine_loader::Capabilities::default().with_voice());

    let mut registry = typio_runtime::core::registry::EngineRegistry::new();
