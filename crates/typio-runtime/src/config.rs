...
impl Config {
    ...
    ///
    /// TOML is the only accepted configuration format. Anything else is
    /// rejected outright rather than coerced through a permissive fallback,
    /// so a malformed file surfaces as an error instead of silently loading
    /// a partially-parsed configuration.
...
mod tests {
    ...
    fn non_toml_input_is_rejected() {
        ...
        // The retired INI-like dialect must not be silently accepted.
        assert!(matches!(
            Config::parse("[engine]\nname = compose\n"),
            Err(ConfigError::Parse)
        ));
